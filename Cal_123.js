$(function() {
    ...
    function calculateSum() {
        var totalSum = 0, dcSum = 0, acSum = 0;
        ...
        associations.forEach(function(assoc) {
            var qty = parseFloat($('#' + assoc.quantity).val()) || 0;
            if ($('#' + assoc.dc).is(':checked')) {
                dcSum += qty;
            } else if ($('#' + assoc.ac).is(':checked')) {
                acSum += qty;
            }
        });

        $('#element_262').val(totalSum);           // Total Quantity
        $('#element_264').val(dcSum + acSum);      // DC + AC Total Quantity
        $('#element_265').val(dcSum);               // DC Total Quantity
        $('#element_266').val(acSum);               // AC Total Quantity
    }

    var allInputs = elementIds.concat(
        associations.flatMap(a => [a.mc, a.dc, a.ac])
    );

    $(allInputs.map(id => '#' + id).join(',')).on('input change', calculateSum);
